    ...
    if has_coord:
        ...
        if root:
            ...
            if ',' not in e.text.replace(',', ' ,').split():
                ...
                if len(conj) > 0:
                    ...
                    if len(second_head) > 0:
                        ...
                        first_complete = [x for x in list(first_head.subtree) if x not in second_complete and x.pos_ != 'CCONJ' and x.text in e.text.split()]
                        root_complete_no_coords = [x for x in list(root.subtree) if x.text in e.text.split() and x not in first_complete + second_complete and x.pos_ != 'CCONJ']

                        if len(root_complete_no_coords) > 0 and len([x.text for x in root_complete_no_coords]) == len(set([x.text for x in root_complete_no_coords])):
                            ...
                            too_add_1 = " ".join([i.text for i in first_complete]) + " " + " ".join([i.text for i in root_complete_no_coords])
                            too_add_2 = " ".join([i.text for i in second_complete]) + " " + " ".join([i.text for i in root_complete_no_coords])
            ...
            if ',' in e.text:
                ...
                list_childs = [x for x in list(root.children) if x.text in e.text.replace(',', ' ,').split() and x.pos_ == 'ADJ']
                print("Hijos:", list_childs)
                root_complete_no_coords = [x for x in list(root.subtree) if x.text in e.text.replace(',', ' ,').split() and x not in list_childs and x.pos_ != 'PUNCT' and x.pos_ != 'CCONJ']
                print("Núcleo:", root_complete_no_coords)

                for yuxt in list_childs:
                    to_add_yuxt = " ".join([i.text for i in list(yuxt.subtree) if i.text in e.text and i not in root_complete_no_coords and i.pos_ != 'PUNCT' and i.pos_ != 'CCONJ']) + " " +  " ".join([i.text for i in root_complete_no_coords])
